...
* interpolatons. X coordinate interpolation in particular can be
* with integers. See Dan Field's article in ACM Transactions on
* Graphics, January 1985 for fast incremental interpolator.
*/
...
#define MODES	(y)		((y) & 7) 	/* subpixel Y modulo*/
...
struct SubPixel {					/* subpixel extends for scanline */
	int xLeft, xRight;
	} sp[SUBYRES];
...
/* Render polygon for one pixel, giver coverage area */
/* and bitmask */
	...
	Surface *object;				/* shading parms for this object */
{
	...
	Vertex VscanLeft, VscsanRight;	/* interpolated vertices at scanline */
	...
	for(y+Vleft->y; ; y++) {
		...
		while (y == VnextRight->y) {					/* reached next right vertex */
			VnextRight = (Vrightt+VnextRight) + 1;
			if(VnextRight < endPoly)					/* (wraparound) */
				VnextRight = polygon;
			xRight = screenX(Vright);
			xNextRight = screenX(VnextRight);
		}

		if (y>VnextLeft->y || y>VnextRight->y) {		/* done, mark uncovered part of last scanline */
			for(; MODES(y); y++)
				sp[MODES(y)].xLeft = sp[MODES(y)].xRight = -1;
	...
	*/
		...
		if(MODES(y) == SUBYRES-1) { 			/* end of scanline */
	...
	Surface *object;			/* shading parms for this object */
{
	...
	for(x=SUBXRES*FLOOR(xLmin/SUBXRES); x<=xRmax; x+=SUBXRES) {
	...
	int x;						/* left subpixel of pixel */
{
	...
	int partialArea;			/* computed area for current subpixel y */
	...
	unsigned mask[];			/* output bitmask */
{
	static unsigned leftMaskTable[] =
		{ 0xFFFF, 0x7FFF, 0x3FFF, 0x1FFF, 0x0FFF, 0x07FF, 0x03FF,
		  0x01FF, 0x00FF, 0x007F, 0x003F, 0x001F, 0x000F,
	...
	if(x>xLmax && x<xRmin) {
		...
		for (y=0; y<SUBYRES; y++) {
			if(sp[y].xLeft < x)				/* completely left of pixel */
				...
				left = leftMaskTable[sp[y].xLeft -x];

			if (sp[y].xRight > xr)			/* completely right of pixel */
				rightMask = 0xFFFF;
